...
def horizontal(video: Path, start, end, out: Path, cpu=False):
    subprocess.run(
...
def vertical(video: Path, start, end, out: Path, srt_name: str, cpu=False):
    ...
    subprocess.run(
        ["ffmpeg", "-y", "-v", "error", "-ss", f"{start:.2f}", "-i", str(video.resolve()), "-t", f"{end - start:.2f}",
